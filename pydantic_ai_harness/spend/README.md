...
`BatchSpendStore` is the protocol `SpendLimits` drives. `add_many` takes every window of one
response as a single operation, each window's share carried by a `SpendEntry`, so a failure
cannot leave the response counted against the day and not the month; `SpendEntry.token` is what
lets a store apply a re-executed accrual once. `SpendStore`, which takes one window per call and
