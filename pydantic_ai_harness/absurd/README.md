...
## Relation to StepPersistence

`AbsurdDurability` and the harness [Step Persistence](../step_persistence/) capability solve
different problems and compose. Absurd gives exactly-once crash-resume *within* a single run: a
worker that dies mid-run picks up from the last completed step. Step Persistence records step events
