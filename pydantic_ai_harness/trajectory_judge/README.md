...
> ```
>
> While Pydantic AI Harness is on 0.x releases, the API may change between minor releases; when it does, deprecation warnings and release-note migration guidance tell you (or your agent) exactly how to upgrade. See the [version policy](https://github.com/pydantic/pydantic-ai-harness#versioning).
...
- Under durable execution (Temporal, DBOS, Prefect) the evaluation is launched from a capability hook, so it runs in orchestration context: it is not checkpointed, and enqueued steering is not persisted across replay. Prefer plain runs for judged work, or treat steering as best-effort there.
