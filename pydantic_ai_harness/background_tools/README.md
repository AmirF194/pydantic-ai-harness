...
- **Result screening**: capabilities that inspect tool results, such as a [Guardrails](https://pydantic.dev/docs/ai/harness/guardrails/) result guard, see the immediate acknowledgment, not the eventual result. A background tool's result is delivered without passing through them, so screen sensitive output inside the tool itself.
