...
uv add "pydantic-ai-harness[playwright]"        # PlaywrightBrowser (real Chromium; run `playwright install chromium`)
...
| | **Playwright Browser** | Real, stateful Chromium via [Playwright](https://playwright.dev/python/) -- navigate, click, type, press keys, select options, hover, scroll, extract text, run JS, screenshot, and inspect console/network on JS-heavy or authenticated pages | :white_check_mark: [Docs](pydantic_ai_harness/playwright/) | [pydantic-deep](https://github.com/vstorm-co/pydantic-deepagents) (vstorm&#8209;co) |
