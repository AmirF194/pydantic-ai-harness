...
`InputGuardrail` rejects prompt injection before any tokens are spent, `OutputGuardrail`
...
def reject_injection(prompt: str) -> GuardrailResult:
    """Refuse messages that try to rewrite the agent's instructions, spending no tokens."""
    if _INJECTION_PATTERNS.search(prompt):
        return GuardrailResult.block("I can only help with product support questions. Let's stick to your issue.")
    return GuardrailResult.allow()


def enforce_policy(output: object) -> GuardrailResult:
...
def build_agent(model: Model | str = DEFAULT_MODEL, customer_id: str = 'anonymous') -> Agent[None, TriageDecision]:
    ...
    return Agent(
        model,
        capabilities=[
            InputGuardrail(guard=reject_injection),
            OutputGuardrail(guard=enforce_policy),
